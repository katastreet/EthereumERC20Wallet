...
public class Ethereum implements Cryptocurrency {
	...
	/**
	 * this blocks get balace(biginteger) in wei (10^18 decimal places)
	 * 
	...
	public static BigInteger getBalance(String address) throws InterruptedException, ExecutionException {
		...
		EthGetBalance ethGetBalance = web3j.ethGetBalance("0x" + address, DefaultBlockParameterName.LATEST).sendAsync()
				.get();
		return ethGetBalance.getBalance();
	}

	/**
	 * this blocks privateKey and to is hex string without 0x prefix
	 * 
	 * @param privateKey
	 * @param to
	 ...
	 * @throws Exception
	 */
	public static boolean transferEther(String privateKey, String to, BigDecimal value)
			throws InterruptedException, IOException, TransactionException, Exception {

		Web3j web3j = Web3j.build(new HttpService(WEB3_URL));
		Credentials node = Credentials.create(privateKey);

		TransactionReceipt transactionReceipt = Transfer.sendFunds(web3j, node, "0x" + to, value, Convert.Unit.ETHER)
				.send();
